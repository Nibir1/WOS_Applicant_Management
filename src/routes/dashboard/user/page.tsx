...
export default function UserPage() {
  ...
  const addNewUser = async (e: { preventDefault: () => void }) => {
    ...
    axios
      .post(
        "http://localhost:5000/authorized_users/insert_user_infos",
        formData
      )
      .then((response) => {
  ...
  useEffect(() => {
    axios
      .get(
        `http://localhost:5000/get_specific_user_infos/${session?.user?.user_email}`
      )
      .then((response) => {
  ...
  const updateUserInfo = async (e: { preventDefault: () => void }) => {
    ...
    axios
      .put(
        `http://localhost:5000/update_user_infos/${session?.user?.user_email}`,
        formData
      )
      .then((response) => {
        if (response.data.Status === "Success") {
  ...
  const deleteUser = async (e: { preventDefault: () => void }) => {
    ...
    if (userToDelete === session?.user?.user_email) {
      ...
      axios
        .delete(`http://localhost:5000/delete_user/${userToDelete}`)
        .then((response) => {
          if (response.data.Status === "Success") {
            console.log("Succeded");
          } else {
