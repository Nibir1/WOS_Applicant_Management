...
export default function HomePage() {
  const updateApplicantInfos_section = useRef(null);
  const ds160Informations_section = useRef(null);
  const goToTop_section = useRef(null);
  ...
  const viewApplicantInformation = async (e: {
    ...
    axios
      .get(
        `http://localhost:5000/get_specific_applicant_infos/${applicant_email}`
      )
      .then((response) => {
      ...
      .catch((error) => setMessage(`Applicant Not Found`));

    // This is to get the ds160 informations
    axios
      .get(`http://localhost:5000/get_ds160_infos/${applicant_email}`)
      .then((response) => {
        console.log({ response: response });
        if (
      ...
      .catch((error) => setMessage(`DS160 Infos Not Filled Yet`));
  };
  ...
  const updateApplicantInfos = async (e: { preventDefault: () => void }) => {
    ...
    axios
      .put(
        `http://localhost:5000/update_applicant_infos/${applicant_email}`,
        formData
      )
      .then((response) => {
  ...
  const deleteApplicant = async (e: { preventDefault: () => void }) => {
    ...
    axios
      .delete(
        `http://localhost:5000/delete_applicant_from_applicants/${applicant_email}`
      )
      .then((response) => {
        if (response.data.Status === "Success") {
          console.log("Succeded");
    ...
    axios
      .delete(
        `http://localhost:5000/delete_applicants_ds160_informations/${applicant_email}`
      )
      .then((response) => {
        if (response.data.Status === "Success") {
          console.log("Succeded");
    ...
    <div className="dashboard-page">
      ...
      <section className="applicant-registration-container">
        <section className="file-input">
          ...
          <img
            src={`http://localhost:5000/images/` + data.applicants_photo}
          ...
          <button
            ...
            onClick={() =>
              handleDownload(
                `http://localhost:5000/images/` + data.applicants_photo
              )
            }
          >
            Download Photo
        ...
        <button
          className="register-applicant"
          style={{ backgroundColor: "#484b4f" }}
          onClick={() => updateApplicantInfos_section.current.scrollIntoView()}
        >
          Update Informations
        </button>
        ...
        <button
          className="register-applicant"
          style={{ backgroundColor: "#484b4f" }}
          onClick={() => ds160Informations_section.current.scrollIntoView()}
        >
          DS160 Informations
        </button>
      ...
      <section ref={ds160Informations_section}>
        ...
        <section className="search-applicant">
          <button onClick={() => goToTop_section.current.scrollIntoView()}>
            Go To Top
          </button>
        </section>
      </section>
